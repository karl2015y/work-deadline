...
const { data: dateNoteList, isFinished } = useAxios<DateNote[]>('/api/taiwan_2023.json')
...
function getDateNote(date: string) {
  const _date = dayjs(date).format('YYYYMMDD');
  const dateNote = find(dateNoteList.value, (_dateNote => _dateNote.西元日期 === _date));
  return dateNote
}
